...
function Import-Package() {
  ...
  if ($nuspec -match '<packageSourceUrl>.+') {
    $nuspec = $nuspec -replace '<packageSourceUrl>.+', "<packageSourceUrl>https://github.com/$GithubRepository/tree/main/automatic/$Name</packageSourceUrl>"
  }
  else {
    Write-Warning "<packageSourceUrl> Was NOT found, Please add manually the following to $Name.nuspec:`n`t<packageSourceUrl>https://github.com/$GithubRepository/tree/main/automatic/$Name</packageSourceUrl>";
  }
...
Import-Package -Name $Name -Version $Version -GithubRepository "jimbrig/choco-packages"